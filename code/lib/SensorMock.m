classdef SensorMock < handle
    ...
    methods
        function self = SensorMock(sensor_type, sensed_objects, sensed_robots, sensor_name)
            ...
            self.sensed_robots = sensed_robots;
        end
        ...
        function r = getRobotPoses(self)
            % a function that returns the poses of all the sensed objcets
            % may not be able to be applied on all objects
            
            try
                disp('i tried');
                r = self.sensed_robots.robot.base;
                disp('i succeded');
            catch ME
                disp('Unable to automatically recieve objects pose \n manual pose extraction must be used');
            end
